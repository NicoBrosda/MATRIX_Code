...
results_path = Path('/Users/nico_brosda/Cyrce_Messungen/Results_260325/EnergyExtrapolation/')
...
axins.text(sim_energy[np.argmax(sim_res)]+0.1, transform_axis_to_data_coordinates(axins, [0.7, 0.03])[1],
        f'{sim_energy[np.argmax(sim_res)]: .2f}$\\,$MeV', fontsize=13, c='red', zorder=3)
ax.indicate_inset_zoom(axins, edgecolor="black")

'''
upper_text = 0.94
param_list, cmap_param, param_unit = sim_energy, energy_cmap, 'MeV'
...
ax.text(*transform_axis_to_data_coordinates(ax, [0.025, upper_text-0.23]),
        ...
        zorder=3, bbox={'facecolor': 'w', 'alpha': 0.8, 'pad': 2, 'edgecolor': 'w'})
'''
format_save(results_path, f'EnergyExtrapolation', legend=False, fig=fig, axes=[ax, axins])

# ---------------- Plot 2: Comparison deposited energy initial E change vs simulated CYRCé wheel --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
energy_colormapper = lambda energy: color_mapper(energy, np.min(sim_energy), np.max(sim_energy))
energy_color = lambda energy: energy_cmap(energy_colormapper(energy))
...
ax.plot(sim_energy_400[0:len(cache_400)], sim_res_400[0:len(cache_400)], c='b', ls='-', marker='')
ax.plot(sim_energy_200[0:len(cache_200)], sim_res_200[0:len(cache_200)], c='r', ls='-', marker='')
...
for i, object in enumerate(sim_energy):
    # ax.errorbar(sim_energy[i], sim_res[i], sim_std[i], c=energy_color(object), marker='', capsize=2, markersize=5, alpha=0.7)
    axins.plot(sim_energy[i], sim_res[i], c=energy_color(object), marker='x', markersize=5,alpha=0.7)
axins.plot(sim_energy_400[0:len(cache_400)], sim_res_400[0:len(cache_400)], c='b', ls='-', marker='|', label='Simulation 400$\\,$µm CYRCé wheel')
axins.plot(sim_energy_200[0:len(cache_200)], sim_res_200[0:len(cache_200)], c='r', ls='-', marker='|', label='Simulation 200$\\,$µm CYRCé wheel')
...
for tick in axins.xaxis.get_major_ticks():
    tick.label1.set_backgroundcolor('white')  # Set background for x-axis tick labels
    # Adjust the size of the background box (e.g., reduce padding)
    tick.label1.set_bbox(dict(facecolor='white', edgecolor='none', pad=1, alpha=0.7))  # Smaller padding for bbox

axins.legend()

format_save(results_path, f'EnergyExtrapolationVsSim', legend=False, fig=fig, axes=[axins, ax])

#----------------------------------------------------------------------------------------------------------------
# How good is this resembled by a proportional relation?
#----------------------------------------------------------------------------------------------------------------
# Interpolate sim data to Cyrcé energies
sim_interp_400 = np.interp(data_wheel_400['energies'][0:len(cache_400)], sim_energy, sim_res)
sim_interp_200 = np.interp(data_wheel_200['energies'][0:len(cache_200)], sim_energy, sim_res)

def proportional(x, a):
    return a*x

popt_400, pcov_400 = curve_fit(proportional, sim_res_400, sim_interp_400)
residuals = sim_interp_400 - proportional(sim_res_400, *popt_400)
ss_res = np.sum(residuals ** 2)
ss_tot = np.sum((sim_interp_400 - np.mean(sim_interp_400)) ** 2)
if ss_tot == 0:
    r_squared_400 = 0
else:
...
popt_200, pcov_200 = curve_fit(proportional, sim_res_200, sim_interp_200)
residuals = sim_interp_200 - proportional(sim_res_200, *popt_200)
ss_res = np.sum(residuals ** 2)
ss_tot = np.sum((sim_interp_200 - np.mean(sim_interp_200)) ** 2)
if ss_tot == 0:
    r_squared_200 = 0
else:
...
# ---------------- Plot 3: Signal current vs deposited energy for 400 um diffuser --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
energy_colormapper = lambda energy: color_mapper(energy, np.min(sim_energy), np.max(sim_energy))
energy_color = lambda energy: energy_cmap(energy_colormapper(energy))
...
ax.plot(sim_energy_400[0:len(cache_400)], sim_res_400[0:len(cache_400)]*popt_400[0], c='b', ls='-', marker='|', label='Simulation 400$\\,$µm CYRCé wheel')
ax.plot(sim_energy_200[0:len(cache_200)], sim_res_200[0:len(cache_200)]*popt_200[0], c='r', ls='-', marker='|', label='Simulation 200$\\,$µm CYRCé wheel')
ax.plot(sim_energy_400[0:len(cache_400)], sim_res_400[0:len(cache_400)], c='b', ls='-', marker='|', alpha=0.3)
ax.plot(sim_energy_200[0:len(cache_200)], sim_res_200[0:len(cache_200)], c='r', ls='-', marker='|', alpha=0.3)
...
print(popt_400[0], np.sqrt(np.diag(pcov_400)))
print(format_value_with_error(popt_400[0], np.sqrt(np.diag(pcov_400))))

text = f"Results fit proportional law"
ax.text(*transform_axis_to_data_coordinates(ax, [0.50, 0.78]), text, fontsize=13, c='k')

text = f"Effectivity {format_value_with_error(popt_400, np.sqrt(np.diag(pcov_400)))} \n R$^2$ = {r_squared_400: .3f}"
ax.text(*transform_axis_to_data_coordinates(ax, [0.60, 0.66]), text, fontsize=13, c='b')

text = f"Effectivity {format_value_with_error(popt_200, np.sqrt(np.diag(pcov_200)))} \n R$^2$ = {r_squared_200: .3f}"
ax.text(*transform_axis_to_data_coordinates(ax, [0.60, 0.54]), text, fontsize=13, c='r')
...
ax.text(*transform_axis_to_data_coordinates(ax, [0.025, upper_text-0.23]),
        ...
        zorder=3, bbox={'facecolor': 'w', 'alpha': 0.8, 'pad': 2, 'edgecolor': 'w'})
'''
format_save(results_path, f'EnergyExtrapolationVsSim_Effectiveness', legend=True)


# ---------------- Plot 4: Experimental response vs simulated response --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
...
# Filter the correct energies from sim and sort for comparison with experimental data
mask = np.isin(sim_energy, data_wheel_200['energies'][0:len(cache_200)])
simII_res_200 = sim_res[mask][np.argsort(sim_energy[mask])[::-1]]

mask = np.isin(sim_energy, data_wheel_400['energies'].to_numpy()[0:len(cache_400)])
simII_res_400 = sim_res[mask][np.argsort(sim_energy[mask])[::-1]]

fig, ax = plt.subplots()
ax.plot(simII_res_200, normed_200, c='r', ls='-', marker='|', label='CYRCé 200$\\,$µm diffuser data')
ax.plot(simII_res_400, normed_400, c='b', ls='-', marker='|', label='CYRCé 400$\\,$µm diffuser data')

'''
for i, object in enumerate(cache_200):
    # ax.errorbar(simII_res_200[i], normed_200[i], std_200[i], sim_std_200[i], c=energy_color(data_wheel_200['energies'][i]), marker='', capsize=5, markersize=7, alpha=0.7)
    ax.plot(simII_res_200[i], normed_200[i], c=energy_color(data_wheel_200['energies'][i]), marker='|', alpha=1)
...
format_save(results_path, f'EnergyCalibNew', legend=True, axes=[ax, axins])


# ---------------- Plot 5: Experimental response vs simulated response --------------------
def check_prop(x_input, y_input, sigma_x=None, sigma_y=None):
    popt, pcov = curve_fit(proportional, x_input, y_input, sigma=sigma_y)
    residuals = y_input - proportional(x_input, *popt)
    ss_res = np.sum(residuals ** 2)
...
c_400II, dc_400II, r2_400II = check_prop(simII_res_400, normed_400, sigma_y=std_400)
c_200II, dc_200II, r2_200II = check_prop(simII_res_200, normed_200, sigma_y=std_200)
print(f'SimII: Effectiveness 400 um diffuser: {c_400II} +/- {dc_400II} with R$^2$ of {r2_400II}')
print(f'SimII: Effectiveness 200 um diffuser: {c_200II} +/- {dc_200II} with R$^2$ of {r2_200II}')
# Assuming we have an effectivity of c given in a current as # electron per incident proton - and if we assume Kleins
# rule has to be valid, this would mean we required 3xEg≈3x3.5eV=10.5 eV to get one electron. This corresponds to an
# effectivity of 1/10.5 electrons / eV. If the effectivity is smaller, then because additional effectivities are
# included in the conversion inbetween simulated energy deposition and measured current.
# Thus: Effectivity C = I * K with I being the ideal effectivity out of Kleins rule (1 electron per 10 eV deposited by
# protons) and K < 1 beeing the additional effectivity (loss).
print(f'The loss is caluclated to be {c_200II/(1/10.5)} respectively {1/10.5 / c_200II} times more protons deposited the expected energy within the SC. This is factor {1/10.5 / c_200II / popt_200} larger as currently predicted by the simulation.')
print(f'The loss is caluclated to be {c_400II/(1/10.5)} respectively {1/10.5 / c_400II} times more protons deposited the expected energy within the SC. This is factor {1/10.5 / c_400II / popt_400} larger as currently predicted by the simulation.')

# To estimate if the geometry factor of the simulation varies for the different apertures of the wheel: Compare
# effectivity pointwise in between no aperture (wheel pos 0) and aperture. If this remains ≈ equal, then the main
# contribution is induced by the outer geometry which can be simulated. If the simulation caches this accurately, this
...
print(f'The loss is caluclated to be {c_200II_cut/(1/10.5)} respectively {1/10.5 / c_200II_cut} times more protons deposited the expected energy within the SC. This is factor {1/10.5 / c_200II_cut / popt_200} larger as currently predicted by the simulation.')
print(f'The loss is caluclated to be {c_400II_cut/(1/10.5)} respectively {1/10.5 / c_400II_cut} times more protons deposited the expected energy within the SC. This is factor {1/10.5 / c_400II_cut / popt_400} larger as currently predicted by the simulation.')

c_400, dc_400, r2_400 = check_prop(sim_res_400, normed_400, sigma_y=std_400)
c_200, dc_200, r2_200 = check_prop(sim_res_200, normed_200, sigma_y=std_200)
print(f'SimI: Effectiveness 400 um diffuser: {c_400} +/- {dc_400} with R$^2$ of {r2_400}')
print(f'SimI: Effectiveness 200 um diffuser: {c_200} +/- {dc_200} with R$^2$ of {r2_200}')
...
ax.plot(simII_res_200, proportional(simII_res_200, c_200II), c='r', ls='--')
ax.plot(simII_res_400, proportional(simII_res_400, c_400II), c='b', ls='--')
...
axins.plot(simII_res_200, normed_200, c='r', ls='-', marker='')
axins.plot(simII_res_400, normed_400, c='b', ls='-', marker='')
axins.plot(simII_res_200, proportional(simII_res_200, c_200II), c='r', ls='--')
axins.plot(simII_res_400, proportional(simII_res_400, c_400II), c='b', ls='--')
axins.set_xlim(x1, x2)
axins.set_ylim(y1, y2)
axins.xaxis.set_major_locator(ticker.AutoLocator())
axins.xaxis.set_minor_locator(ticker.AutoMinorLocator())
...
format_save(results_path, f'EnergyCalibFit', legend=True, axes=[ax, axins])

# ---------------- Plot 6: Extrapolation to higher energies (lower energy deposition) Zoom --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
energy_colormapper = lambda energy: color_mapper(energy, np.min(data_wheel_200['energies'][0:len(cache_200)]), np.max(data_wheel_200['energies'][0:len(cache_200)]))
energy_color = lambda energy: energy_cmap(energy_colormapper(energy))
...
ax.plot(sim_res[sim_energy >= 25], proportional(sim_res[sim_energy >= 25], c_200II_cut), c='r', ls=':')
ax.plot(sim_res[sim_energy >= 25], proportional(sim_res[sim_energy >= 25], c_400II_cut), c='b', ls=':')
...
format_save(results_path, f'EnergyCalibExtrapolation', legend=True, axes=[ax])

# ---------------- Plot 7: Extrapolation to higher energies (now vs energy) --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
energy_colormapper = lambda energy: color_mapper(energy, np.min(data_wheel_200['energies'][0:len(cache_200)]), np.max(data_wheel_200['energies'][0:len(cache_200)]))
energy_color = lambda energy: energy_cmap(energy_colormapper(energy))
...
ax.plot(data_wheel_200['energies'][0:len(normed_200)], proportional(simII_res_200, c_200II), c='r', ls='--')
ax.plot(data_wheel_400['energies'][0:len(normed_400)], proportional(simII_res_400, c_400II), c='b', ls='--')

ax.plot(sim_energy[sim_energy >= 25], proportional(sim_res[sim_energy >= 25], c_200II_cut), c='r', ls=':')
ax.plot(sim_energy[sim_energy >= 25], proportional(sim_res[sim_energy >= 25], c_400II_cut), c='b', ls=':')
...
format_save(results_path, f'EnergyCalibExtrapolationII', legend=True, axes=[ax])

# ---------------- Plot 8: Extrapolation to higher energies (now vs energy) --------------------
energy_cmap = sns.color_palette("crest_r", as_cmap=True)
energy_colormapper = lambda energy: color_mapper(energy, np.min(sim_energy), np.max(sim_energy))
energy_color = lambda energy: energy_cmap(energy_colormapper(energy))
...
# ax.plot(data_wheel_200['energies'][0:len(normed_200)], proportional(simII_res_200, c_200II), c='r', ls='--')
# ax.plot(data_wheel_400['energies'][0:len(normed_400)], proportional(simII_res_400, c_400II), c='b', ls='--')
ax.plot(sim_energy, proportional(sim_res, c_200II_cut), c='k', ls='-')
ax.plot(sim_energy, proportional(sim_res, c_400II_cut), c='k', ls='-')
...
for i, energy in enumerate(energies):
    ...
    print(energy, res)
    print(transform_data_to_axis_coordinates(ax, [energy, res]))
    if i == 0:
        res_0 = res
...
format_save(results_path, f'EnergyCalibExtrapolationII_Zoom', legend=False, axes=[ax])