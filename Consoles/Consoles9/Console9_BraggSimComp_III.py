...
def plot_maps_sim(save_path, comp_list, map_cache, signal_cache, param_color, shape_position, run_name,
                  param_unit='MeV', add_wedge=True, keep_scale=True, wedge_thick=10):
...
def plots_sim_comp(save_path, comp_list, signal_cache, material_depth, material_depth_sim, signal_height,
    ...
    ax2 = ax.twinx()
    ax.set_xlabel('Position y Experiment (mm)')
    ax.set_ylabel(f'Signal Current per initial proton ({scale_current})')
    ...
    for k in range(len(signal_cache)):
        fig, ax = plt.subplots()
        # ax2 = ax.twinx()
        # ax2.set_zorder(ax.get_zorder() - 1)
        ax.patch.set_visible(False)
        ...
        for i, obj in enumerate(signal_cache[:k]):
            wheel_position = obj[0]
            line = obj[2]
            y_pos = obj[1]
            param = comp_list[i]
        ...
        if k == 0:
            ...
            gradient_arrow(ax, transform_axis_to_data_coordinates(ax, [0.1, 0.925]),
                           transform_axis_to_data_coordinates(ax, [0.1, approx_len]),
                           cmap=cmapII, lw=10, zorder=5)
            ax.text(*transform_axis_to_data_coordinates(ax, [0.025, 0.94]),
                    f'{np.max(comp_list): .2f}$\\,${param_unit}', fontsize=13, c=param_color(np.max(comp_list)),
                    zorder=4, bbox={'facecolor': 'w', 'alpha': 0.8, 'pad': 2, 'edgecolor': 'w'})
            ax.text(*transform_axis_to_data_coordinates(ax, [0.035, approx_len - 0.085]),
                    f'{param: .2f}$\\,${param_unit}', fontsize=13, c=param_color(param),
                    zorder=4, bbox={'facecolor': 'w', 'alpha': 0.8, 'pad': 2, 'edgecolor': 'w'})

        format_save(save_path, save_name=f'{k}ConsecWedge', save=True, legend=False, legend_position=3, fig=fig)
    ...
    ax2 = ax.twinx()
    ax.set_xlabel('Proton energy (MeV)')
    ax.set_ylabel(f'Signal Current per intial proton ({scale_current})')
    ax2.set_ylabel(f'Deposited Energy per initial proton ({scale_sim}eV)')
