...
print(super_resolution_matrix_correct_red(4))
plt.error
# Axis limits
...
for i in range(num_small_boxes):
    ...
    rect = plt.Rectangle(((i+0)*(box_size/2), 2), box_size/2, box_size,
                         facecolor=color, edgecolor='black')
    ax.add_patch(rect)
...
for i in range(num_small_boxes):
    ...
    rect = plt.Rectangle(((i+0)*(box_size/2), 1), box_size/2, box_size,
                         facecolor=color, edgecolor='black')
    ax.add_patch(rect)
...
just_save(Path('/Users/nico_brosda/Cyrce_Messungen/Style') / 'SuperRes', 'Logo_SuperResFormulaCorrect', plot_size=plot_size)
