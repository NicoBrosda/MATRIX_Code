...
def align_and_compare_images(
    ...
    def pad_image(image, target_shape):
        pad_y_before = (target_shape[0] - image.shape[0]) // 2
        pad_y_after = target_shape[0] - image.shape[0] - pad_y_before
        pad_x_before = (target_shape[1] - image.shape[1]) // 2
        pad_x_after = target_shape[1] - image.shape[1] - pad_x_before

        padded = np.pad(image,
                        ((pad_y_before, pad_y_after),
                         (pad_x_before, pad_x_after)),
                        mode='constant', constant_values=0)
        return padded, (pad_y_before, pad_x_before)

    image_high_res_padded, pad_high = pad_image(image_high_res_resampled, max_shape)
    image_low_res_padded, pad_low = pad_image(image_low_res_resampled, max_shape)
    ...
    return diff_image, alignment_score, (rotation, center_shift, func_calls, pad_high, pad_low,
                                         image_high_res_padded, transformed_low_res)


def align_and_compare_images2(
    image_low_res,
    image_high_res,
    pixel_size_low,
    pixel_size_high,
    ...
    """
    Align the downsampled high-resolution image to the low-resolution image.
    """
    ...
    def pad_image(image, target_shape):
        ...
        padded = np.pad(image,
                        ((pad_y_before, pad_y_after),
                         (pad_x_before, pad_x_after)),
                        mode='constant', constant_values=0)
        return padded, (pad_y_before, pad_y_after, pad_x_before, pad_x_after)
    ...
    # --- Change here: transform the high-res image, not the low-res one ---
    def alignment_metric(params):
        global func_calls
        func_calls += 1
        rotation, shift_x, shift_y = params
        transformed_high_res = transform_image(image_high_res_padded, rotation, [shift_x, shift_y])
        valid_mask = (image_low_res_padded > 0) | (transformed_high_res > 0)
        high_res_valid = transformed_high_res[valid_mask]
        low_res_valid = image_low_res_padded[valid_mask]
    ...
    return diff_image, alignment_score, (rotation, center_shift, func_calls, pad_high, pad_low,
                                         transformed_high_res, image_low_res_padded)