...
for j, folder_path in enumerate([folder_image1, folder_image2, folder_image3]):
    ...
    old_files = array_txt_file_search(os.listdir(image_path), searchlist=[phrase], file_suffix='.csv', txt_file=False)
    [os.remove(image_path / file) for file in old_files]
    ...
    for file in files:
        ...
        if i == 0 and j == 1:
            continue
        if j <= 1:
            rename = f'{i}_{phrase}_x_{(i % 7) * 5 + 50:.0f}_y_{(i // 7) * 5 + 50:.0f}.csv'
        else:
            rename = f'{phrase}_x_{(i % 3) * 2 + 50:.0f}_y_{(i // 3) * 2 + 50:.0f}.csv'
            # rename = f'0{i}_{phrase}_x_{(i // 3) * 10 + 50:.0f}_y_{(i % 3) * 10 + 50:.0f}.csv'
