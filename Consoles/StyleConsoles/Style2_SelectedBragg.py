...
def plots_vs_wedge_position(save_path, comp_list, signal_cache, material_depth, signal_height, signal_pos,
    ...
    gradient_arrow(ax, transform_axis_to_data_coordinates(ax, [0.1, 0.755]),
                   transform_axis_to_data_coordinates(ax, [0.1, 0.885]),
...
# ----------------------------------------------------------------------------------------------------------------
# '''
comp_list = data_wheel_200['energies'].to_numpy()[1:14]
param_unit = 'MeV'
signal_cache = signal_cache_200[1:14]
map_cache = wedge_200[1:14]
shape_position = bragg_pos_wedge200
save_path = results_path / 'Wedge200Reduced/'
