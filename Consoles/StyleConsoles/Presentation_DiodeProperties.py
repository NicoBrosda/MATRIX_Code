...
print({k: plt.rcParams[k] for k in ['axes.labelsize','xtick.labelsize','ytick.labelsize']})
...
fig, ax = plt.subplots(1, 1, figsize=fullsize_plot)
...
for i, measure in enumerate(measurements):
    if i == 0:
        data_frame = pd.read_csv(folder_path / measure, sep=' ', header=None, skiprows=21,
                                 names=['Mean64', 'Mean1', 'Sigma64', 'Sigma1', 'Date'])
    else:
        data_frame = pd.concat([data_frame, pd.read_csv(folder_path / measure, sep=' ', header=None, skiprows=0,
                                                        names=['Mean64', 'Mean1', 'Sigma64', 'Sigma1', 'Date'])],
                               ignore_index=True)
...
class TimeFormat:
    ...
    def value(self):
        return (
                    self.year * 365 * 24 * 60 * 60 + self.month * 30 * 24 * 60 * 60 + self.day * 24 * 60 * 60 + self.hour * 60 * 60 + \
...
ax.text(*transform_axis_to_data_coordinates(ax, [0.5, 0.96]), r'\textbf{2.4$\,$MGy total dose | 43.6$\,$Gy/s} ',
         fontsize=16, ha='center',
         va='top', color='r')

format_save(save_path=results_path, save_name=f"LongtermIrradiation", dpi=dpi, plot_size=fullsize_plot,
            save_format=save_format, fig=fig, legend=False)
# ------------------------------------------------------------------------------------------------------------------
# Ax2: Linearity
# ------------------------------------------------------------------------------------------------------------------
fig, ax2 = plt.subplots(1, 1, figsize=fullsize_plot)
...
readout, position_parser, voltage_parser, current_parser = lambda x, y: ams_channel_assignment_readout(x, y,
                                                                                                       channel_assignment=channel_assignment), standard_position, standard_voltage, lambda \
    c: current3(c, conversion_factor=0.688)
...
dark_crit = ['exp2_']
readout, position_parser, voltage_parser, current_parser = lambda x, y: ams_channel_assignment_readout(x, y,
                                                                                                       channel_assignment=channel_assignment), standard_position, standard_voltage, current4
B = Analyzer((1, 128), (0.4, 0.4), (0.1, 0.1), readout, position_parser, voltage_parser, current_parser)
B.scale = 'nano'

currents2, fit_currents2, signal2, fit2, std2, fit_std2, fit_r22, std_r22 = linearity_return(folder_path2, crit,
                                                                                             dark_crit, B)
...
ax.text(*transform_axis_to_data_coordinates(ax, [0.98, 0.15]),
        r'Before $\bar{\mathrm{R}}^2$' + ' = {x:.5f}'.format(x=fit_r2), fontsize=16,
        ha='right', va='bottom', c='r')
ax.text(*transform_axis_to_data_coordinates(ax, [0.98, 0.14]),
        r'After $\bar{\mathrm{R}}^2$' + ' = {x:.5f}'.format(x=fit_r22), fontsize=16,
        ha='right', va='top', c='b')

ax.legend(loc='upper left', fontsize=16)

format_save(save_path=results_path, save_name=f"LinearityAfter", dpi=dpi, plot_size=fullsize_plot,
            save_format=save_format, fig=fig, legend=False)
# ------------------------------------------------------------------------------------------------------------------
# Ax3: Beam Shape
# ------------------------------------------------------------------------------------------------------------------
fig, ax3 = plt.subplots(1, 1, figsize=fullsize_plot)
ax = ax3

signal_comparison_channel(folder_path1, results_path, ['exp6_25nA_nA_1.9_x_0_y_0', 'exp12_25nA_12_12_2024_16_04_42'],
                          'exp8_dark_without_cup_nA_1.9_x_0_y_0', A, names=['Before Irradiation', 'After irradiation'],
                          normed=True, mark_n=[], add_plot=ax, save_plot=False, lw=1.5, color_list=['r', 'orange'])
signal_comparison_channel(folder_path2, results_path, ['exp3_nA_1.9_x_28.25_y_0.0'], 'exp2_nA_1.9_x_28.25_y_0.0', B,
                          names=['3 months later'], add_plot=ax, normed=True, lw=1.5, color_list=['blue'],
                          save_plot=False)
...
format_save(save_path=results_path, save_name=f"BeamShapeAfter", dpi=dpi, plot_size=fullsize_plot,
            save_format=save_format, fig=fig, legend=False)
# ------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------------------------
