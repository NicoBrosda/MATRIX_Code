...
from Plot_Methods.plot_standards import AngleAnnotation, arc_between_points, curved_line_through_points
...
class LineShape:
    ...
    def get_angle(self):
        ...
        dy = np.max(self.points[:, 1]) - np.min(self.points[:, 1])
        print(dx, dy)
    ...
    def add_angle_indicator(
        ...
        # Thin edge = first point
        x0, y0 = self.points[np.argmin(self.points[:, 0])]
        y0 = 0
    ...
    def add_to_plot(self, y_min=0.2, y_max=0.8, ax=None, add_angle=False, text_fmt="{:.1f}°", *args, **kwargs):
        ...
        y_max_raw = np.max(y_raw)
        height = y_max_raw - y_min_raw
        if height == 0:
            raise ValueError("Shape has zero thickness")
        # scale both top and bottom
        y_top = (y_raw - y_min_raw) / height * (y_max - y_min) + y_min
        y_bottom = np.full_like(y_top, y_min)
        y_top = [transform_axis_to_data_coordinates(ax, [0.5, j])[1] for j in y_top]
        y_bottom = transform_axis_to_data_coordinates(ax, [0.5, y_min])[1]
        fill = ax.fill_between(self.points[:, 0], y_top, y_bottom, *args, **kwargs)
        '''
        return fill
...
def homogenize_pixel_size(input_map):
    ...
    new_z = []
    for i, col in enumerate(input_z.T):
        if x_steps[i] > pixel_size + 1e-12:  # use ceil to avoid truncation from floating precision
